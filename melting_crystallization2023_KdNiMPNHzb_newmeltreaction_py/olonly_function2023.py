...
# only target to calculate MgO, FeO, MnO and NiO
# Jan 17, 2023
...
def get_firstT_olonly(clcm_olonly,P,molarSiO2_adjust):
    ...
    #constantB = 0.03014*clcm_olonly['Na2O']+0.1454*clcm_olonly['K2O']+0.010076*P-3.1174
    ...
    #TK = fsolve(lambda tk: constantA*math.exp(6604/tk+constantB)+constantC*math.exp(6604/tk+constantB)*math.exp((-3766-6000*constantA/66.67*math.exp(6604/tk+constantB))/(8.3144*tk)+constantD)-66.67, 1600)
    T = TK[0]-273.15
    return T
...
def NiMn_olonly(T,cm_kdMg_oll_olonly,clppm_olonly,f_step_olonly,olppm_olonly,clcm_olonly,cm_kdFe2_oll_olonly,Po):
    #wt_kdNi_oll_olonly = (3.346*cm_kdMg_oll_olonly-3.665)*1.09 ## original Beattie equation, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    #wt_kdNi_oll_olonly = math.exp(4505/(T+273.15)-2.075)*(cm_kdMg_oll_olonly*1.09) ## original Matzen equation, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    #wt_kdNi_oll_olonly = math.exp(4288/(T+273.15)+0.01804*(clcm_olonly['SiO2']*cm_tot*cm_mass['SiO2']/100)-2.8799)*(cm_kdMg_oll_olonly*1.09) ## new equation fitted by MPN dataset, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    # wt_kdNi_oll_olonly = math.exp(4449/(T+273.15)+0.01137*(clcm_olonly['SiO2']*cm_tot*cm_mass['SiO2']/100)-2.6345)*(cm_kdMg_oll_olonly*1.09) ## new equation fitted by Hzb dataset, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    wt_kdNi_oll_olonly = math.exp(4272/(T+273.15)+0.01582*(clcm_olonly['SiO2']*cm_tot*cm_mass['SiO2']/100)-2.7622)*(cm_kdMg_oll_olonly*1.09) ## new equation fitted by MPN+Hzb dataset, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    #wt_kdNi_oll_olonly = math.exp(4146/(T+273.15)+0.01559*(clcm_olonly['SiO2']*cm_tot*cm_mass['SiO2']/100)-2.6742)*(cm_kdMg_oll_olonly*1.09) ## new equation fitted by 80% MPN+Hzb dataset, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    ...
    #wt_kdMn_oll_olonly = math.exp(0.00877960466828*(cm_tot*40.32*clcm_olonly['MgO']/100)-1.50316580917181)*(cm_kdMg_oll_olonly*1.09) ## original Matzen equation, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    if Po < 30:
