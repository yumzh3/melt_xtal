# all melting functions used in melting
# Jan 16, 2023
...
def liquid_wt_isoequ(source_wt,f,f_mineral,P,T,cl_wt,cl_cm,bulkD,res,Po):  
    cl_wt['K2O'] = source_wt['K2O']/(bulkD['K2O']*(1-f)+f)
    bulkD['Na2O'] = 0.008+10**((5100+73*P)/(T+273.15)-4.48)*f_mineral['cpx']*0.01  
...
def KDFeMg(T,P,f_step,cl_wt,cl_cm,res,cl_molar,ol):  
    ...
    #kdMgO_oll_cm = math.exp(6604/(T+273.15)+0.03014*cl_cm['Na2O']+0.1454*cl_cm['K2O']+0.010076*P-3.1174)  # refit Lamgnuir et al. 1992 with MPN dataset
...
def TPF_polyfrac(P,f,mgnumber_source,Po):  
    ...
    T = 13*P+1140+600*(1-P/88)*f+20*(mgnumber_source-89) # mantle solidus refer to Langmuir et al. 1992
...
# calculate fraction of sulfide and residual sulfur during polybaric fractional melting
# refer to Zhao etal 2022
...
# calculate fraction of sulfide and residual sulfur during isobaric equilibrium melting
# refer to Zhao etal 2022
    ...
    if Po >= 30:  # here we assume that when pressure is higher than 30 kbar, garnet will converse to spinel in addition to the melting reactions
        if f_mineral['gt'] > 0:
            if P > 30:  # calculate the garnet-spinel conversion factor
                ...
                gt_factor = 1  # when there is garnet remainning in the mineral, 30gt+8ol+81cpx=19opx+100melt (Walter 1998, 3 GPa grt-out) and 100gt+20ol=23sp+60opx+37cpx (garnet converse to spinel)
...
def mineral_phase_isoequ(Po,P,f,source_phase,source_phase2,f_gt0,source_phase3,source_phase4,f_cpx0,f_sp0,f_mineral):  
    if Po >= 30:  # when pressure is higher than 30 kbar, we assume there is garnet in the source but with no spinel
        if source_phase['gt'] > 0 and f_mineral['gt'] > 0:  # when there is garnet remainning, 30gt+8ol+81cpx=19opx+100melt (Walter 1998, 3 GPa grt-out)
...
# calculate Ni in the liquid and olivine during polyfrac when considering sulfur furing melting
def Ni_polyfrac_S(T,kdMgO_oll_cm,cl_wt,f_mineral,res,f_step,bulkD,ol,f_sulfideliquid,f_sulfide):
    ...
    #kdNi_wt['oll'] = (3.346*kdMgO_oll_cm-3.665)*1.09 ## original Beattie equation, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    #kdNi_wt['oll'] = math.exp(4505/(T+273.15)-2.075)*(kdMgO_oll_cm*1.09)  ## original Matzen equation, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    #kdNi_wt['oll'] = math.exp(4288/(T+273.15)+0.01804*cl_wt['SiO2']-2.8799)*(kdMgO_oll_cm*1.09) ## new equation fitted by MPN dataset, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    #kdNi_wt['oll'] = math.exp(4449/(T+273.15)+0.01137*cl_wt['SiO2']-2.6345)*(kdMgO_oll_cm*1.09) ## new equation fitted by Hzb dataset, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    kdNi_wt['oll'] = math.exp(4272/(T+273.15)+0.01582*cl_wt['SiO2']-2.7622)*(kdMgO_oll_cm*1.09) ## new equation fitted by MPN+Hzb dataset, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    #kdNi_wt['oll'] = math.exp(4146/(T+273.15)+0.01559*cl_wt['SiO2']-2.6742)*(kdMgO_oll_cm*1.09) ## new equation fitted by 80% MPN+Hzb dataset, *1.09 to convert from cmf to wt%, observed from Walter 1998 
...
# calculate Ni in the liquid and olivine during polyfrac melting without constraint from sulfur
...
# calculate Ni in the liquid and olivine during isoequ when considering sulfur furing melting
...
# calculate Ni in the liquid and olivine during isoequ melting without constraint from sulfur
...
def Mn_polyfrac(T,kdMgO_oll_cm,cl_wt,f_mineral,res,f_step,bulkD,ol,Po,kdFeO_oll_cm):  
    ...
    #kdMn_wt['oll'] = 0.78
    #kdMn_wt['oll'] = (0.259*kdMgO_oll_cm-0.049)*1.09
    #kdMn_wt['oll'] = math.exp(-2.76+3583/T)
    #kdMn_wt['oll'] = math.exp(0.00877960466828*cl_wt['MgO']-1.50316580917181)*(kdMgO_oll_cm*1.09) ## original Matzen equation, *1.09 to convert from cmf to wt%, observed from Walter 1998 
    ## Le Roux etal 2011 Table 3 for lowP-T opx and cpx, Davis etal 2013 GCA Table 13 for highP-T opx, cpx, and grt, sp
    if Po < 30:
        kdMn_wt['oll'] = 0.79*kdFeO_oll_cm*1.09
        # kdMn_wt['cpxl'] = 1.11 # lowP-T melting
        # kdMn_wt['opxl'] = 0.75 # lowP-T melting
        kdMn_wt['cpxl'] = 0.85 # lowP-T melting
        kdMn_wt['opxl'] = 0.7 # lowP-T melting
        kdMn_wt['gtl'] = 1.241 # lowP-T melting
        kdMn_wt['spl'] = 0.46 # lowP-T melting
    else:
        kdMn_wt['oll'] = 0.79*kdFeO_oll_cm*1.09
        kdMn_wt['cpxl'] = 0.768 # highP-T melting
        kdMn_wt['opxl'] = 0.640 # highP-T melting
        kdMn_wt['gtl'] = 1.241 # highP-T melting
        kdMn_wt['spl'] = 0.46 # highP-T melting 
